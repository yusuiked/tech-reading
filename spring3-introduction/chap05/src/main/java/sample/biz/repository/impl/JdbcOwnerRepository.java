...
public class JdbcOwnerRepository implements OwnerRepository {
    ...
    @Override
    public Owner insert(String ownerName) {
        jdbcTemplate.update("INSERT INTO OWNER (OWNER_NAME) VALUES (?)", ownerName);
        return jdbcTemplate.queryForObject("SELECT * FROM OWNER WHERE OWNER_NAME=?", new BeanPropertyRowMapper<>(Owner.class), ownerName);
    }
}
